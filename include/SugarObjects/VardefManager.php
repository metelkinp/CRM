...
class VardefManager
{
    ...
    /**
     * this method is a solution for 'user management' problem in ppt custom modules.
     * Method add an entry into acl_actions database with default settings.
     * @param $module
     */
    static function addEntryToAclActions($module)
    {
        global $ACLActions;
        $db = $GLOBALS['db'];
    ...
    static function createVardef($module, $object, $templates = array('default'), $object_name = false)
    {
        ...
        if (strpos($module, 'PPT_') !== false) {
            self::addEntryToAclActions($module);
        }
    }
